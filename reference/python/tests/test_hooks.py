...
class TestMain:
    ...
    def test_dispatches_pull_on_start(self, clan_dir_json):
        ...
        stdin = io.StringIO("{}")
        stdout = io.StringIO()
        with patch.object(sys, "argv", ["hermes.hooks", "pull_on_start"]), \
    ...
    def test_dispatches_exit_reminder(self, clan_dir_json):
        ...
        stdin = io.StringIO("{}")
        stdout = io.StringIO()
        with patch.object(sys, "argv", ["hermes.hooks", "exit_reminder"]), \
             patch.object(sys, "stdin", stdin), \
             patch.object(sys, "stdout", stdout), \
             patch("hermes.hooks._default_clan_dir", return_value=clan_dir_json):
            main()
